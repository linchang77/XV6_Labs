...
1. **如何读取帧指针**：
   - **困难描述**：这个函数需要读取帧指针来遍历调用栈，但是不知道如何调用帧指针。
   - **解决办法**：查看提示，有函数 `r_fp()` 可以正确读取帧指针，于是简单了解了汇编语言，看懂了代码。
2. **不理解fp = *(uint64 *)(fp-16);**：
   - **困难描述**：在代码中提到了fp-16。
   - **解决办法**：涉及到RISC-V 架构栈帧布局：
局部变量（Local Variables）：
位于栈帧的顶部。其大小根据具体函数的局部变量数量和类型而定。

保存的帧指针（Saved FP）：
位于 FP - 16 处，保存了前一个帧指针的值。在进入函数时，当前帧指针（FP）会被保存到这个位置。

返回地址（Return Address）：
位于 FP - 8 处，保存了调用该函数的位置地址。在调用一个函数时，返回地址会被保存到这个位置，以便函数执行完毕后能够返回到正确的位置。。
...
本实验的目的是在 xv6 中实现一个定时器报警功能，使得用户进程能够在每隔一定的 CPU 时间片（tick）后调用一个用户定义的处理函数。通过实现 `sigalarm` 和 `sigreturn` 系统调用，用户进程可以在每次处理函数调用后恢复到原来的执行位置，从而实现周期性操作或其他功能。为 xv6 添加一个功能，使其在进程使用 CPU 时间时定期发出警报。这对希望限制自身 CPU 时间消耗的计算密集型进程，或希望在计算的同时进行一些周期性操作的进程可能会很有用。更广泛地说，你将实现一种用户级中断/故障处理的原始形式；你可以使用类似的方法在应用程序中处理页面错误。
...
1. **理解用户进程中断处理的机制**：
   - **困难描述**：如何实现用户程序的终端。
   - **解决办法**：查阅资料，了解到通过修改myproc中的alarm_ticks，alarm_handler等参数。
